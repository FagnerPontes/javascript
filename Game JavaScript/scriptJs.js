...
class shooter {
	constructor(parent, myWidth, myHeight, myColor, positionY, paused) {
		...
		this.element.style.setProperty('position', 'fixed'); //css -> posição fixa
		this.element.style.setProperty('left', '50%');//css -> posição inicial no eixo x
		this.element.style.setProperty('top', `${positionY}px`);//css -> posição no eixo y
		this.element.style.setProperty('width', `${myWidth}px`);//css -> largura do objeto
		this.element.style.setProperty('height', `${myHeight}px`);//css -> altura do obejto
		this.element.style.setProperty('background-color', myColor);//css -> cor de fundo do objeto
		this.parent.appendChild(this.element);//inserir elemento
		window.addEventListener('mousemove', (e) => {//movimentar objeto com o mouse
			if (!this.paused)
				this.element.style.setProperty('left', `${e.clientX - (myWidth / 2)}px`);//centralizar objeto na posição do mouse
		});
	}
	destroi() {//função para remover o projétil.
		this.element.style.setProperty('display', 'none');//ocultar elemento
...
class bullet {
	constructor(parent, myWidth, myHeight, myColor, positionX, positionY) {
		...
		this.element.style.setProperty('position', 'fixed');//css -> posição fíxa	
		this.element.style.setProperty('left', `${this.position.x}px`);//css -> posição no eixo x
		this.element.style.setProperty('top', `${this.position.y}px`);//css -> posição no eixo y
		this.element.style.setProperty('width', `${myWidth}px`);//css -> largura do objeto bullet
		this.element.style.setProperty('height', `${myHeight}px`);//altura do objeto bullet
		this.element.style.setProperty('background-color', myColor);//css -> cor de fundo do objeto bullet
		this.element.style.setProperty('border-top-left-radius', '50%');//css -> arredondamento superior esquerdo
		this.element.style.setProperty('border-top-right-radius', '50%');//css -> arredondamento superior direito
		this.parent.appendChild(this.element);//inserir elemento
	}
	...
	move() {//função para mover o projétil
		if (this.position.y > 0) {//se a posição no eixo y for maior que 0
			...
			this.element.style.setProperty('top', `${this.position.y}px`);//atualize a posição
		}
	}

	destroi() {//função para remover o projétil.
		this.position.y = -200;//posição fora da tela em 200 pixels
		this.element.style.setProperty('top', `${this.position.y}px`);//atualizar a posição
		this.element.style.setProperty('display', 'none');//ocultar elemento
		this.element.remove();//remover elemento
	}
}
...
class enemy {
	constructor(parent, myWidth, myHeight, myColor, windowWidth, windowHeight) {
		...
		this.element.style.setProperty('left', `${this.position.x}px`);//css -> posição no eixo x
		this.element.style.setProperty('top', `${this.position.y}px`);//css -> posição no eixo y
		this.element.style.setProperty('width', `${myWidth}px`);//css -> largura do objeto enemy
		this.element.style.setProperty('height', `${myHeight}px`);//css -> altura do objeto enemy
		this.element.style.setProperty('background-color', myColor);//css -> cor de fundo do objeto enemy
		this.element.style.setProperty('border-radius', '50%');//css -> arredondamento do objeto enemy
		this.parent.appendChild(this.element);//inserir elemento
	}
	...
	destroi() {//função para remover o inimigo.
		...
		this.element.style.setProperty('top', `${this.position.y}px`);//atualizar posição
		this.element.style.setProperty('display', 'none');//ocultar elemento
		this.element.remove();//remover elemento
	}
}
...
var startGameEvent = (event) => {
	if (event.key == 'Enter') {//se pressionar Enter
		document.body.innerText = '';//conteúdo do elemento body
		myShooter = new shooter(document.body, 20, 20, '#0000ff', myWindowHeight - 150);//objeto atirador
		window.addEventListener('click', newBulletEvent);//evento de criação de um novo projétil
		time = Date.now(); //delay para criar um novo inimigo:
		updateFrame(myInterval);//função responável pela animação
		document.removeEventListener('keydown', startGameEvent);//remover evento startGameEvent
	}
}

//função responável pela animação
...
document.addEventListener('keydown', (event) => {
	if (event.key == ' ') {//se a tecla Space for pressionada
		...
		(isPaused) ?//se isPaused for vedadeiro
			window.removeEventListener('click', newBulletEvent) ://remova o evento newBulletEvent
			window.addEventListener('click', newBulletEvent);//senão, adicione o evento newBulletEvent
	}
});

document.body.style.backgroundColor = '#000';//css -> cor de fundo preto
document.body.style.color = '#fff';//css -> cor da fonte branca